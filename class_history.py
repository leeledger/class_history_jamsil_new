...
def auto_lessons():
    ...
    # 'subject_list_detail.html' 템플릿으로 결과를 전달하며 렌더링
    return render_template('auto_lessons.html', 
                           subjectdetails=subject_details, 
                           get_subject_by_id=get_subject_by_id,
...
def report_sample():
    ...
    if platform.system() == 'Windows':
        photo_directory = r"\\172.30.1.18\nas1\학원공유"  # 윈도우 환경의 파일 경로
    else:
        photo_directory = '/volume1/NAS1/학원공유'  # 리눅스 환경의 파일 경로
    filtered_photos = []
...
@app.route('/api/insert_lessons', methods=['POST'])
def insert_lessons():
    try:
        data = request.get_json()
        ...
        if not lessons_data:
            raise ValueError("등록할 수업 데이터가 없습니다.")

        created_lessons = []

        for lesson_data in lessons_data:
            # 필수 데이터 검증
            subject_detail_id = int(lesson_data['subject_detail_id'])
            date_str = lesson_data['date'].strip()
            ...
            db.session.flush()  # lesson_id 생성을 위한 flush
        ...
        return jsonify({
            'status': 'success',
            'message': f'{len(created_lessons)}개의 수업이 등록되었습니다.',
